...
/// Return the path to the directory in which cached models should be saved.
///
/// Defaults to `~/.cache/ocrs/`. Creates the directory if it doesn't exist.
...
/// needed. If the source is a local path, it will be loaded directly.
...
/// Resolve a model source: use a local path if the file exists, otherwise
/// fall back to the given URL for auto-download.
pub fn resolve_model_source<'a>(local_path: &'a str, fallback_url: &'a str) -> ModelSource<'a> {
    if path::Path::new(local_path).exists() {
        ModelSource::Path(local_path)
    } else {
        ModelSource::Url(fallback_url)
    }
}
